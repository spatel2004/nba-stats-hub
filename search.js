...
async function loadStarPlayers() {
  ...
  try {
    const giannisresponse = await fetch(`/api/handler?userInput=${encodeURIComponent(giannis.fullName)}&season=2025`);
    const giannisData = await giannisresponse.json();

    const lebronresponse = await fetch(`/api/handler?userInput=${encodeURIComponent(lebron.fullName)}&season=2025`);
    const lebronData = await lebronresponse.json();

    const curryresponse = await fetch(`/api/handler?userInput=${encodeURIComponent(curry.fullName)}&season=2025`);
...
async function fetchPlayerStats(playerName) {
  try {
    const response = await fetch(`/api/handler?userInput=${encodeURIComponent(playerName)}&season=2025`);
    if (!response.ok) {
